...
export function Toggle({ id, checked, onChange }: ToggleProps): JSX.Element {
  return (
    ...
    >
      <span
        aria-hidden="true"
        className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-[var(--color-toggle-knob)] shadow-[var(--shadow-sm)] transition-transform duration-150 ${
